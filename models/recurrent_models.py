...
class LSTMGenerator(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, n_layers=1, hidden_dim=256):
        ...
        self.lstm = nn.LSTM(in_dim, hidden_dim, n_layers, batch_first=True).cuda()
        self.linear = nn.Sequential(nn.Linear(hidden_dim, out_dim), nn.Tanh()).cuda()
...
class LSTMDiscriminator(nn.Module):
    ...
    def __init__(self, in_dim, n_layers=1, hidden_dim=256):
        ...
        self.linear = nn.Sequential(nn.Linear(hidden_dim, 1), nn.Sigmoid()).cuda()
